function [optimal_d, error, ConfusionMatrix] = Exercise2(d_max)
    ...
    for dCount = 1:2:d_max
        ...
        [C(:,:,dCount),order] = confusionmat(labels_Test,classified_labels);
        error_Classification(dCount) = (10000-trace(C(:,:,dCount)))/100;

    end

    figure;
    hold off;
    stem(1:2:size(error_Classification),error_Classification(error_Classification~=0));grid on; hold all;
    ...
    stem(15,error_Classification(15),'linewidth',2,'marker','x','markersize',10);
    ylabel('classification error (%)');
    title('Classification error as a function of eigen depth in PCA');
    legend('error','min-error','verification');
    optimal_d = error_Classification(index_min);
    error = error_Classification(index_min);
    ConfusionMatrix = C(:,:,index_min);
end